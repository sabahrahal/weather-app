...
export const SmallCard: FC<Card> = ({ time, temperature, temperatureMin, temperatureMax, icon, description, isLink, searchResult, latitude, longitude }) => {
  ...
  return (
    <ParentElement to={`/forecast/daily?search=${searchResult}&latitude=${latitude}&longitude=${longitude}`} className='flex flex-col items-center justify-center text-center dark:bg-gray-600 rounded-lg p-2'>
        ...
        <span className='text-xs dark:text-dark-text-400'>{description}</span>
        {
          temperature === undefined
            ...
            : <div className='flex items-center justify-center'>
                <span className='text-[40px] font-mono dark:text-dark-text-200'>{temperature}</span>
                <sup className='text-[10px] dark:text-dark-text-300 font-mono'>°C</sup>
              </div>
        }
