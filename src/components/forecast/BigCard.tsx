...
export const BigCard: FC<Card> = ({ time, date, temperature, temperatureMin, temperatureMax, icon, description, isLink, searchResult, latitude, longitude }) => {
  ...
  return (
    <ParentElement to={`/forecast/daily?search=${searchResult}&latitude=${latitude}&longitude=${longitude}`} className='flex order-first flex-col xs:flex-row dark:bg-gray-600 rounded-lg p-2 col-span-2 md:col-span-3 items-center justify-between drop-shadow-md hover:drop-shadow-none duration-150'>
      ...
      <div className='flex flex-col items-center xs:items-start xs:w-1/3'>
        ...
        />
        <span className='dark:text-dark-text-200'>{ name }</span>
        {
          date === undefined
            ? <span className='dark:text-dark-text-300 text-sm'>{ time }</span>
            : <span className='dark:text-dark-text-300 text-sm'>{ date }</span>
        }
      </div>
      ...
      { temperature === undefined
        ? <>
            ...
            <div className='flex items-center justify-center'>
              ...
              <span className='text-[60px] font-mono dark:text-dark-text-100'>{temperatureMin}</span>
              <sup className='text-[20px] dark:text-dark-text-300 font-mono'>°C</sup>
            </div>
          </>
      ...
      <div className='xs:w-1/3 text-center xs:text-right'>
        ...
        <span className='text-sm dark:text-dark-text-300'>{description}</span>
      </div>
  </ParentElement>
  )
}
